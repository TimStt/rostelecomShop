...
import { useGetStateOnLocalStorage } from "../../hooks/use-get-state-LS";
...
export const useAddProductBasket = ({
  ...
  let store =
    typeof localStorage !== "undefined"
      ? (JSON.parse(
          localStorage?.getItem(storeName as string) as string
        ) as unknown as IBasketGoods[] | IFavoritesGoods[])
      : [];
  ...
  return () => {
    ...
    !isAuth && localStorage.setItem(storeName as string, JSON.stringify(store));
